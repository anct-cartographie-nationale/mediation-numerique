...
export class MapModule {
  ...
  public constructor(
    ...
    @Optional() @Inject(GEO_JSON_TOKEN) private readonly geoJsonService: GeoJsonRepository
  ) {
    if ([geometryPolygonConfiguration, markerType, zoomLevel, geoJsonService].includes(null)) {
      throw new Error(
        'Cannot import `MapModule` without calling `forRoot` with valid parameters: you must provide defined `geometryPolygonConfiguration`, `markerType`, `zoomLevel` and `geoJsonService`.'
      );
    }
  }
}
